...
export function ConfirmedDialog() {
  ...
  const hour = getFormattedDate(dayLib().startOf('day').add(selectData.startTime!, 'm'));
  ...
  return <div>
    ...
    <div className="mb-4">
      ...
      <ul className="list-disc list-inside">
        {selectData.services!.map((service) => {
          ...
          return (
            <li key={service.name}>
              {service.name} - <span className="font-semibold">R$ {service.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
            </li>
          );
        })}
      </ul>
    ...
    <div className="mb-4">
      <strong>Total:</strong>
      <span className="text-lg font-bold"> R$ {totalPrice.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
    </div>
    ...
    <div className="mb-4">
      ...
      <Input id='forPersonName' placeholder='Nome' value={selectData.forPersonName} onChange={(e) => handleSelectData({ forPersonName: e.target.value })} />
    </div></div>;
