...
export function NewScheduleProvider({
  children
  ...
  const [selectData, setSelectData] = useState<IData>({ professionalId: 1, isIgnoreBreak: false });
  ...
  const { data: clients, refetch: refetchClients, isLoading: isClient } = useQuery({
    queryKey: ['clients', queryClient],
    queryFn: () => ClientService.getClients(queryClient),
  ...
  const handleQueryClient = ({ search = '', take = 5 }) => {
    setQueryClient({ search, take });
    refetchClients();
  };

  const handleSelectData = (newData: IDataSelect) => {
    setSelectData((old) => {
      return { ...old, ...newData };
    });
  ...
  const createSchedule = () => {
    ...
    BookingService.createBooking({
      ...
      endTime: selectData.startTime! + selectData.services!.reduce((acc, service) => acc + service.durationMinutes, 0),
  ...
  return (
    <NewScheduleProviderContext.Provider value={{
      categories,
      clients,
      handleQueryClient, handleSelectData, setStep, isClient
      , isValid, selectClient, selectData, services, step, handleOpen, isOpen,
      createSchedule, isCreate
    }}>
      {children}
