...
export function Header({ selectWorkingDay }: IHeader) {
  ...
  return dayLib(selectWorkingDay.date).isSame(dayLib(), 'day') ? <div className="w-full flex items-center justify-between">
    ...
    <NewScheduleDialog />
  </div> : <div className="text-xl font-bold mb-4 text-center">
    {dayLib(selectWorkingDay.date).format('DD/MM/YYYY')}
  </div>;
}
