...
import { FormData } from '@/view/pages/BookingHour';
...
export function ConfirmedHourStep() {
  const navigate = useNavigate();
  const [isCreate, setIsCreate] = useState(false);
  const form = useFormContext<FormData>();

  const hour = getFormattedDate(
    dayLib(form.getValues('confirmedStep.date'))
      .startOf('day')
      .add(form.getValues('confirmedStep.startTime'), 'm'),
  );
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside">
            <li>
              {form.getValues('serviceStep.name')} -{' '}
              <span className="font-semibold">
                {form.getValues('serviceStep.price').toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                })}
              </span>
        ...
        <Button
          ...
          onClick={async () => {
            ...
            form.setValue('confirmedStep', {
              ...
              date: form.getValues('confirmedStep.date'),
              startTime: form.getValues('confirmedStep.startTime'),
              endTime: form.getValues('confirmedStep.endTime'),
              professionalId: 1,
              services: [
                {
                  serviceId: form.getValues('serviceStep.serviceId'),
                  order: 1,
                  price: form.getValues('serviceStep.price'),
                },
              ],
