...
export function NewScheduleBaseDialog() {
  const { isValid, step, setStep, isOpen, handleOpen, createSchedule, isCreate } =
    useNewSchedule();
  const [dialogData, setDialogData] = useState({
    title: 'Novo Agendamento',
    description:
      'Selecione o cliente, o serviço e o horário para realizar o agendamento.',
  });
  ...
  const content = useMemo(() => {
    ...
    if (step === 4) {
      setDialogData({
        ...
        description: 'Selecione o horário disponível para o agendamento.',
      });
      ...
      <DialogContent className="w-full max-w-[425px] sm:max-w-lg">
        ...
        <DialogFooter className="flex-row justify-between sm:justify-between">
          ...
          <div className="flex gap-2">
            <Button disabled={step === 0} variant="outline" onClick={() => setStep(0)}>
              Início
            </Button>
            <Button
              disabled={!isValid || isCreate}
              onClick={() => {
                if (step === 5) {
                  createSchedule();
                } else {
