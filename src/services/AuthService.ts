...
export class AuthService {
  static async signUp({ name, password,
    ...
    const { data } = await httpClient.post('/auth/sign-up', { name, password, phone, username });

    return data;
  }

  static async signIn({ password, username }: ISignInDTO) {
    const { data } = await httpClient.post<ISignInResponse>('/auth/sign-in', { password, username });
