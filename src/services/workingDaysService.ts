...
export class WorkingDaysService {
  ...
  static async retrieveWorkingDay(id: number) {
    const { data } = await httpClient.get<{ result: IWorkingDay }>(`/working-days/${id}`);
  ...
  static async getFreeTime({ date, professionalId, requiredMinutes, isIgnoreBreak }: IFreeTimeDTO) {
    const { data } = await httpClient.post<
      {
        'display': string,
        'total': number
      }[]
    >('/working-days/free-time', { date, professionalId, requiredMinutes, isIgnoreBreak });
